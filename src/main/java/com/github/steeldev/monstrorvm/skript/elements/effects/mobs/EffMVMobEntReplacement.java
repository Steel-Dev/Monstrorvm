...
public class EffMVMobEntReplacement extends Effect {

    static {
        Skript.registerEffect(EffMVMobEntReplacement.class, "add %entitytypes% to entities to replace [on spawn] of %mvmob%");
    }
    ...
    protected void execute(Event event) {
        ...
        if (MobManager.getMob(m.key) != null) {
            ...
            for(EntityType type : entityTypes.getArray(event)){
                m.withEntityToReplace(Util.convertEntityTypeFromSkriptToBukkit(type));
            }
        }
    }
    ...
    public String toString(@Nullable Event event, boolean b) {
        return "add " + entityTypes.getArray(event) + " to entities to replace [on spawn] of " + mob.toString(event,b);
    }
